    ...
    if boton1:        
        ...
        if len(records) == 0:
            try:
                ...
                #url = "https://imdb.com"
                #url = f"https://www.imdb.com/find/?q={busqueda.lower()}&s=tt&ref_=fn_tt_pop"
                ...
                soup = BeautifulSoup(response.text, "html.parser")                
                buscador = soup.find("input", id="suggestion-search")                
                if buscador is not None:
                    st.warning("pass 1")
                    buscador["value"] = busqueda
                else:
                    st.warning("buscador fallo 1")
                
                form = soup.find("form", id="nav-search-form")
                
                if form is not None and form.get("action"):
                    st.warning("inicio form 2")
                    ...
                    st.warning(data)
                    response = requests.get(url + action_url, params=data, headers=headers)                
                    if response.status_code == 200:
                        st.warning("inicio 2")
                        soup = BeautifulSoup(response.text, "html.parser")
                        ...
                        st.warning(f"status 2: {response.status_code}")
                    # Esperar un tiempo para que se carguen los resultados
                else:
                    st.warning("nones form")
                sleep(3)

                # Encontrar el botón de "Más resultados" y hacer clic si está presente
                #boton = soup.select_one(".find-see-more-title-btn > button:nth-child(1)")
                boton = soup.select_one(".find-see-more-title-btn button")
                
                st.warning(boton)
                if boton is not None:
                    # Realizar una nueva solicitud GET al enlace del botón
                    st.warning("inicio boton 2")
                    st.warning(boton)
                    response = requests.get(url + boton["href"], headers=headers)
                    soup = BeautifulSoup(response.text, "html.parser")
                """
